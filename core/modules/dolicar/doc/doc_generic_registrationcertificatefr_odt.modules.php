...
/**
 *	\file       htdocs/core/modules/dolicar/doc/doc_generic_registrationcertificatefr_odt.modules.php
 *	\ingroup    dolicar
 ...
 */
class doc_generic_registrationcertificatefr_odt extends ModelePDFRegistrationCertificateFr
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'DOLICAR_REGISTRATIONCERTIFICATEFR_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="param1" value="DOLICAR_REGISTRATIONCERTIFICATEFR_ADDON_PDF_ODT_PATH">';
		...
		$texte .= $conf->global->DOLICAR_REGISTRATIONCERTIFICATEFR_ADDON_PDF_ODT_PATH;
		$texte .= '</textarea>';
		...
		if (!empty($conf->global->DOLICAR_REGISTRATIONCERTIFICATEFR_ADDON_PDF_ODT_PATH)) {
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'];
				$texte .= ' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=dolicar_registrationcertificatefr/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
		...
		$texte .= '<input type="hidden" value="DOLICAR_REGISTRATIONCERTIFICATEFR_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *	@param		RegistrationCertificateFr	$object				Object source to build document
	...
	public function write_file($object, $outputlangs, $srctemplatepath, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->dolicar->dir_output) {
			...
			$dir = $conf->dolicar->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->dolicar->dir_temp);
				if (!is_writable($conf->dolicar->dir_temp)) {
					$this->error = "Failed to write in temp directory ".$conf->dolicar->dir_temp;
				...
				try {
					$odfHandler = new odf(
						$srctemplatepath,
						array(
						'PATH_TO_TMP'	  => $conf->dolicar->dir_temp,
