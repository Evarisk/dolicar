...
if ($conf->global->DOLICAR_API_REMAINING_REQUESTS_COUNTER <= 0) {
    ...
    exit;
} elseif ($conf->global->DOLICAR_API_REMAINING_REQUESTS_COUNTER <= 100) {
...
if (is_object($registrationCertificateObject)) {
    ...
    if ($resultProductlot <= 0) {
        ...
        $product->correct_stock_batch($user, $conf->global->DOLICAR_DEFAULT_WAREHOUSE_ID, 1,0, $langs->transnoentities('ClientVehicle'), 0, '', '', $productLot->batch, '', 'dolicar_registrationcertificate', 0);
