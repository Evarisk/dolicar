...
/**
 *  \file       registrationcertificatefr_document.php
 *  \ingroup    dolicar
 *  \brief      Tab for documents linked to RegistrationCertificateFr
 */
...
$hookmanager->initHooks(array('registrationcertificatefrdocument', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->dolicar->multidir_output[$object->entity ? $object->entity : $conf->entity]."/registrationcertificatefr/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->dolicar->registrationcertificatefr->write; // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'document', $langs->trans("RegistrationCertificateFr"), -1, $object->picto);
	...
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->dolicar->registrationcertificatefr->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;

	//$relativepathwithnofile='registrationcertificatefr/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile = 'registrationcertificatefr/'.dol_sanitizeFileName($object->ref).'/';
