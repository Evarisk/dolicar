...
$permissiontoread          = $user->rights->dolicar->read;
		...
		if (!$error) {
			...
			if (!empty(GETPOST('title'))) {
				...
				if (!$error && $projectID > 0) {
					...
					$task->ref        = $defaultref;
					$task->label      = (!empty($conf->global->DOLICAR_TASK_LABEL_VALUE) ? $conf->global->DOLICAR_TASK_LABEL_VALUE : $langs->trans('CommercialFollowUp')) . ' - ' . $project->title;
					$task->date_c     = dol_now();
...
if ($permissiontoaddthirdparty) {
	...
	// Name, firstname
	if ($conf->global->DOLICAR_THIRDPARTY_NAME_VISIBLE > 0) {
	...
	if ($conf->global->DOLICAR_THIRDPARTY_CLIENT_VISIBLE > 0) {
		print '<tr><td class="titlefieldcreate fieldrequired"><label for="name">' . $langs->trans('ProspectCustomer') . '</label></td>';
		print '<td>' . $formcompany->selectProspectCustomerType(GETPOSTISSET('client') ? GETPOST('client') : $conf->global->DOLICAR_THIRDPARTY_CLIENT_VALUE, 'client', 'customerprospect', 'form', 'maxwidth200 widthcentpercentminusx') . '</td>';
	}

	// Phone
	if ($conf->global->DOLICAR_THIRDPARTY_PHONE_VISIBLE > 0) {
	...
	// Email
	if ($conf->global->DOLICAR_THIRDPARTY_EMAIL_VISIBLE > 0) {
	...
	// Web
	if ($conf->global->DOLICAR_THIRDPARTY_WEB_VISIBLE > 0) {
	...
	// Private note
	if ($conf->global->DOLICAR_THIRDPARTY_PRIVATE_NOTE_VISIBLE > 0 && isModEnabled('fckeditor')) {
	...
	// Categories
	if (isModEnabled('categorie') && $conf->global->DOLICAR_THIRDPARTY_CATEGORIES_VISIBLE > 0 ) {
...
if ($permissiontoaddcontact) {
	...
	// Name, firstname
	if ($conf->global->DOLICAR_CONTACT_LASTNAME_VISIBLE > 0) {
	...
	if ($conf->global->DOLICAR_CONTACT_FIRSTNAME_VISIBLE > 0) {
	...
	// Job position
	if ($conf->global->DOLICAR_CONTACT_JOB_VISIBLE > 0) {
	...
	// Phone
	if ($conf->global->DOLICAR_CONTACT_PHONEPRO_VISIBLE > 0) {
	...
	// Email
	if ($conf->global->DOLICAR_CONTACT_EMAIL_VISIBLE > 0) {
...
if ($permissiontoaddproject) {
	...
	// Label
	if ($conf->global->DOLICAR_PROJECT_LABEL_VISIBLE > 0) {
	...
	if (!empty($conf->global->PROJECT_USE_OPPORTUNITIES)) {
		// Opportunity status
		if ($conf->global->DOLICAR_PROJECT_OPPORTUNITY_STATUS_VISIBLE > 0) {
			print '<tr><td><label for="opp_status">' . $langs->trans('OpportunityStatus') . '</label></td>';
			print '<td>' . $formproject->selectOpportunityStatus('opp_status', GETPOSTISSET('opp_status') ? GETPOST('opp_status') : $conf->global->DOLICAR_PROJECT_OPPORTUNITY_STATUS_VALUE, 1, 0, 0, 0, '', 0, 1) . '</td>';
			print '</tr>';
		}
		...
		if ($conf->global->DOLICAR_PROJECT_OPPORTUNITY_AMOUNT_VISIBLE > 0) {
			...
			print '<td><input type="text" name="opp_amount" id="opp_amount" size="5" value="' . dol_escape_htmltag(GETPOSTISSET('opp_amount') ? GETPOST('opp_amount') : $conf->global->DOLICAR_PROJECT_OPPORTUNITY_AMOUNT_VALUE) . '"></td>';
			print '</tr>';
		}
	}

	// Date start
	if ($conf->global->DOLICAR_PROJECT_DATE_START_VISIBLE > 0) {
	...
	// Categories
	if (isModEnabled('categorie') && $conf->global->DOLICAR_PROJECT_CATEGORIES_VISIBLE > 0) {
