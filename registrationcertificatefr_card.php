...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->dolicar->registrationcertificatefr->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/dolicar/registrationcertificatefr_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/dolicar/registrationcertificatefr_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->dolicar->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('dolicar:RegistrationCertificateFr', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('registrationcertificatefr'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', dol_buildpath('/dolicar/registrationcertificatefr_agenda.php', 1).'?id='.$object->id);
	...
	$diroutput = $conf->dolicar->dir_output;
	$trackid = 'registrationcertificatefr'.$object->id;
